...
from Scripts.MetaModule import *
...
class ResNet(MetaModule):
    def __init__(self, block, num_blocks, num_classes=2):
        ...
        self.conv1 = MetaConv2d(4, 64, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = MetaBatchNorm2d(64)
        self.layer1 = self._make_layer(block, 64, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2)
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=2)
        self.linear = MetaLinear(1024 * block.expansion, num_classes)
