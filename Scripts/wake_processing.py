...
def get_files():
	...
	full_data_5 = loadmat(path + '\\day_08_28_pythonfile.mat')["a5m"]
	full_data_4 = loadmat(path +'\\day_08_28_pythonfile.mat')["a4m"]
	full_data_3 = loadmat(path + '\\day_08_28_pythonfile.mat')["a3m"]
	full_data_1 = loadmat(path + '\\day_08_28_pythonfile.mat')["a1m"]
	full_times = loadmat(path + '\\day_08_28_pythonfile.mat')['tm']
	start_time = full_times[0,0]
	full_times = np.round((full_times - start_time)*24*3600)
	for file in files:
		if file.endswith("pythonfile.mat"):
			if file != 'day_08_28_pythonfile.mat':
	...
	data_series_5 = np.append(np.flip(full_data_5, 1)/max_response, full_times.reshape(full_times.shape[0], 1), axis = 1).transpose()
	data_series_4 = np.append(np.flip(full_data_4, 1)/max_response, full_times.reshape(full_times.shape[0], 1), axis = 1).transpose()
	data_series_3 = np.append(np.flip(full_data_3, 1)/max_response, full_times.reshape(full_times.shape[0], 1), axis = 1).transpose()
	data_series_1 = np.append(np.flip(full_data_1, 1)/max_response, full_times.reshape(full_times.shape[0], 1), axis = 1).transpose()
