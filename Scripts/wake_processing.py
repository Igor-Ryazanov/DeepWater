...
def frames_by_timestamps(data_series, wake_start_ts, depth = 28, datapoints = 60):
    wake_samples = []
    depth_ind = [True]*depth + [False]*(28-depth) + [True]
    for ws in wake_start_ts:
        ind = np.where(data_series[:,-1,:]==ws)[1]
        if ind.size !=0:
            ind2 = int(ind[0])+datapoints
            wake_samples.append(data_series[:,depth_ind,int(ind[0]):ind2])
